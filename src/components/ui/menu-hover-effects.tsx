...
export function MenuHoverLink({ href, children, className }: MenuHoverLinkProps): React.JSX.Element {
  return (
    <Link href={href} className={cn("relative inline-block group", className)}>
      ...
      <span
        aria-hidden="true"
        className="absolute inset-0 border-t-2 border-b-2 border-ink scale-y-[2] opacity-0 origin-center transition-[scale,opacity] duration-300 ease-out group-hover:scale-y-100 group-hover:opacity-100"
      />
      {/* Background fill animation */}
      <span
        aria-hidden="true"
        className="absolute top-[2px] left-0 w-full h-full bg-ink scale-0 opacity-0 origin-top transition-[scale,opacity] duration-300 ease-out group-hover:scale-100 group-hover:opacity-100"
      />
    </Link>
  );
}
